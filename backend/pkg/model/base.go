...
// | `json:"field,omitempty"` | Пропустить поле, если оно имеет нулевое значение ("", 0, nil и т.п.). |
// | `json:"-"`     		  | Поле игнорируется полностью (не сериализуется и не десериализуется).  |
// | `json:"field,string"`    | Преобразует значение в строку (например, число → строка в JSON)..  |
